...
async def dispatch_openai_requests(messages_list, model: str,
                                   temperature: float):
    ...
    """
    async_responses = [
        openai.ChatCompletion.acreate(model=model,
                                      messages=x,
...
def dispatch_single_openai_requests(message, model: str, temperature: float):
    """Dispatches requests to OpenAI API asynchronously.
